...
public class RegistrationService {
    ...
    private final FamilyAccessGateway familyAccessGateway;
    ...
    public RegistrationService(
            ...
            FamilyAccessGateway familyAccessGateway,
            PluginConfig config,
            Logger logger,
            WebhookService webhookService) {
        this.ipAnalyticsRepository = ipAnalyticsRepository;
        this.familyAccessGateway = familyAccessGateway;
        ...
        for (RegistrationValidator validator : validators) {
            ...
            if (!result.isValid()) {
                if ("IP_IN_USE".equals(result.getErrorCode()) && result.getConflict() != null) {
                    boolean sameFamily = familyAccessGateway.areInSameGroup(
                            request.getNickname(),
                            result.getConflict()
                    );
